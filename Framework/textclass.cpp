...
bool TextClass::SetIndexCount(int indexCount, ID3D11DeviceContext* deviceContext)
{
	char tempString[16];
	char indexCountString[16];
	bool result;


	// Convert the index integer to string format.
	_itoa_s(indexCount, tempString, 10);

	// Setup the cpu string.
	strcpy_s(indexCountString, "Index = ");
	strcat_s(indexCountString, tempString);

	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence, indexCountString, 20, 60, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetObjectCount(int objectCount, ID3D11DeviceContext* deviceContext)
{
	...
	// Setup the cpu string.
	strcpy_s(objectCountString, "Object = ");
	strcat_s(objectCountString, tempString);

	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence, objectCountString, 20, 80, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
...
bool TextClass::SetResolution(int width, int height, ID3D11DeviceContext* deviceContext)
{
	...
	_itoa_s(height, tempString2, 10);

	// Setup the cpu string.
	strcpy_s(resString, "Resolution = ");
	strcat_s(resString, tempString);
	strcat_s(resString, "X");
	strcat_s(resString, tempString2);


	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence, resString, 20, 100, 0.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}
