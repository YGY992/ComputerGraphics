						  ...
						  float screenDepth, float screenNear)
{
	...
	// --- 깊이 검사 활성화 상태 생성 ---
// depthEnabledStencilDesc는 기본적으로 깊이 검사 활성화 상태와 비슷하게 설정
	...
	result = m_device->CreateDepthStencilState(&depthEnabledStencilDesc, &m_depthEnabledStencilState);
	if (FAILED(result))
	{
		return false;
	}
	...
	depthDisabledStencilDesc.DepthEnable = false;           // 깊이 검사 비활성화
